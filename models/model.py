...
class StaticFusion(nn.Module):
    def __init__(self, sta_dim=32, out_size=32, num_heads=4, sparsity=0.2):
        ...
        self.learnable_token = nn.Parameter(torch.randn(1, 1, out_size) * 0.02)
        ...
        self.ln = nn.Linear(sta_dim, out_size)
    ...
    def forward(self, x_static):
        ...
        token = self.learnable_token.expand(B, -1, -1)  # (B, 1, out_size)

        gate_input = torch.cat([q_base, token], dim=-1)  # (B, 1, out_size*2)
        gate = torch.sigmoid(self.token_gate(gate_input))  # (B, 1, 1)
        
        q = q_base + gate * token
        
        q = self.query_proj(q)  # (B, 1, out_size)
        k = self.key_proj(x_proj)  # (B, 1, out_size)
        v = self.value_proj(x_proj)  # (B, 1, out_size)

        head_dim = self.out_size // self.num_heads
        q = q.reshape(B, 1, self.num_heads, head_dim).permute(0, 2, 1, 3)
        k = k.reshape(B, 1, self.num_heads, head_dim).permute(0, 2, 1, 3)
        v = v.reshape(B, 1, self.num_heads, head_dim).permute(0, 2, 1, 3)
        ...
        attn_output = attn_output.permute(0, 2, 1, 3).reshape(B, 1, self.out_size)
        attn_output = self.out_proj(attn_output + x_proj)
        out = self.norm(attn_output + x_proj)
        out = self.relu(out)
...
class FBGPredictor(nn.Module):
    def __init__(self, hidden_size=32, hidden_size_d=64, num_layers1=1, num_layers2=2, 
        ...
        self.beta = nn.Sequential(
            nn.Linear(hidden_size, 16)  # Generate single weight parameter
        )
        # Progressive Fusion
    ...
    def forward(self, id, sta, ni, label):
        ...
        # static
        b = id.shape[0]
        h_sta, A = self.static_fusion(self.ln(sta))
        ...
        for i in range(his_fbg.shape[0]):
            if true_counts_per_batch[i] > 1:
                b_his_fbg = his_fbg[i, :true_counts_per_batch[i]] 
